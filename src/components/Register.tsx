...
import { Eye, EyeOff, Mail, Lock, User, Phone, ArrowRight, TrendingUp, CheckCircle, Calendar, Globe, Users } from 'lucide-react';
...
const Register: React.FC<RegisterProps> = ({ onSwitchToLogin, onClose }) => {
  ...
  try {
    const response = await fetch('https://backend-silvofinance.onrender.com/api/auth/register', {
      method: 'POST',
      headers: {
    ...
    onSwitchToLogin("Inscription réussie ! Veuillez vous connecter."); // Redirige vers login
  } catch (error: any) {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl relative max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-8">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600">Rejoignez plus de 8 000 investisseurs satisfaits</p>
            {formData.referralCode && (
              <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-lg">
                <div className="flex items-center justify-center space-x-2">
                  <Users className="w-5 h-5 text-green-600" />
                  <span className="text-green-800 font-medium">
                    Invité par le code : <span className="font-bold">{formData.referralCode}</span>
                  </span>
                </div>
            ...
            <div className="space-y-1">
              <label className="flex items-start space-x-3">
                ...
                <span className="text-sm text-gray-600 leading-relaxed">
                  ...
                  . Je confirme avoir plus de 18 ans et comprendre les risques liés aux investissements.
                </span>
              </label>
              {errors.terms && <p className="text-red-500 text-sm pl-8">{errors.terms}</p>}
            </div>
          ...
          <div className="mt-4 p-4 bg-blue-50 rounded-lg">
            <p className="text-xs text-blue-800 text-center">
              🔒 Inscription sécurisée SSL • Données cryptées • Conforme RGPD • Régulé BCEAO
            </p>
          </div>
        </div>
      </div>
