...
import { 
  ArrowLeft, TrendingUp, Shield, Star, Calendar, 
  Calculator, CheckCircle, Zap, Crown, Gem, 
  Clock, Target, Building, Truck, ShoppingCart, 
  Bitcoin, PieChart, AlertCircle, Loader
} from 'lucide-react';
...
const PackDetailPage: React.FC = (
    ...
    const fetchPackDetails = async () => {
      try {
        ...
        const mappedPack: PackData = {
          ...
          dailyReturn: packFromApi.min_amount * packFromApi.interest_rate / 100,
          totalReturn: packFromApi.min_amount + (packFromApi.min_amount * packFromApi.interest_rate / 100 * packFromApi.duration_days),
  ...
  const getPackSectors = (name: string): Sector[] => {
    const sectorsMap: Record<string, Sector[]> = {
      'Mini Starter': [
        { name: 'E-commerce & Livraisons', percentage: 40, icon: ShoppingCart, description: 'Investissement dans les plateformes e-commerce locales' },
        { name: 'Transport', percentage: 30, icon: Truck, description: 'Financement de flottes de transport' },
        { name: 'Finance & Crypto', percentage: 20, icon: Bitcoin, description: 'Trading algorithmique et crypto' },
        { name: 'Immobilier', percentage: 10, icon: Building, description: 'Immobilier locatif à court terme' }
      ],
      // Ajoutez les autres packs...
    };
    return sectorsMap[name] || [
      {
        name: 'Finance & Crypto',
        percentage: 40,
        icon: Bitcoin,
  ...
  const getPackAdvantages = (pack: PackDetails): string[] => {
    ...
    return [
      ...
      `Gains quotidiens de ${formatCurrency(totalReturn  / 40)} a ${formatCurrency(maxTotalReturn / 40)}`,
  ...
  const maxTotalReturn = packDetails.max_amount * (1 + packDetails.return_percentage_40_days / 100);
  
  const amount = investment > 0 ? investment : packDetails.min_amount;
  const dailyReturn = amount * (packDetails.interest_rate / 100);
  const totalReturn = packDetails.min_amount * (1 + packDetails.return_percentage_40_days / 100);
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-900 mb-4">Chargement du pack...</h1>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-900 mb-4">Erreur lors du chargement du pack</h1>
  ...
  if (!packDetails) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-900 mb-4">Pack non trouvé</h1>
          <Link to="/packs" className="text-blue-600 hover:text-blue-700">
            Retour aux packs d'investissement
          </Link>
        </div>
  ...
  if (investmentAmount < packDetails.min_amount) {
    setError(`Le montant minimum est de ${formatCurrency(packDetails.min_amount)}`);
    return;
  }

  if (investmentAmount > packDetails.max_amount) {
    setError(`Le montant maximum est de ${formatCurrency(packDetails.max_amount)}`);
    return;
  }
  ...
  return (
    <>
      <div className="min-h-screen bg-gray-50">
        {/* Header */}
        <div className={`bg-gradient-to-r ${packDetails.color} text-white py-16`}>
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex items-center space-x-6 mb-8">
              ...
              <div>
                <h1 className="text-4xl md:text-5xl font-bold mb-2">{packDetails.name}</h1>
                <p className="text-xl text-white/90">{packDetails.description}</p>
              </div>
            </div>

            <div className="grid md:grid-cols-4 gap-6">
              <div className="bg-white/10 backdrop-blur-md rounded-lg p-4 text-center">
                <div className="text-2xl font-bold">{packDetails.interest_rate}%</div>
                <div className="text-sm text-white/80">Par jour</div>
              </div>
              <div className="bg-white/10 backdrop-blur-md rounded-lg p-4 text-center">
                <div className="text-2xl font-bold">{packDetails.period} jours</div>
                <div className="text-sm text-white/80">Période</div>
              </div>
              <div className="bg-white/10 backdrop-blur-md rounded-lg p-4 text-center">
                <div className="text-2xl font-bold">{formatCurrency(selectedAmount || packDetails.min_amount)} - {formatCurrency(selectedAmount || packDetails.max_amount)}</div>
                <div className="text-sm text-white/80">Investissement</div>
              </div>
              <div className="bg-white/10 backdrop-blur-md rounded-lg p-4 text-center">
                <div className="text-2xl font-bold">{packDetails.riskLevel}</div>
            ...
            <div className="lg:col-span-2 space-y-12">
                    ...
                    <div key={index} className="border border-gray-200 rounded-lg p-6">
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center space-x-3">
                          <div className={`w-12 h-12 bg-gradient-to-r ${packDetails.color} rounded-lg flex items-center justify-center`}>
                        ...
                        <div className="text-right">
                          ...
                          <div className="text-sm text-gray-500">du portefeuille</div>
                        </div>
                      </div>
              ...
              <div className="bg-white rounded-xl shadow-lg p-8">
                ...
                <div className="bg-purple-50 p-6 rounded-lg">
                  <p className="text-lg text-purple-800 font-semibold mb-2">{packDetails.targetProfile}</p>
                  <p className="text-purple-700">
                    Ce pack est spécialement conçu pour les investisseurs recherchant un rendement quotidien 
                    de {packDetails.interest_rate}% avec doublement du capital en seulement {packDetails.period} jours.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-8">
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <div className="text-2xl font-bold">
                      {formatCurrency(packDetails.min_amount)} - {formatCurrency(packDetails.max_amount)}
                    </div>
                    <div className="text-sm text-white/80">Plage d'investissement</div>
                  </div>

                  {/* Résultats */}
                  <div className="bg-gradient-to-r from-green-50 to-blue-50 p-6 rounded-lg border border-green-200">
                    <h4 className="font-semibold text-gray-900 mb-4">Résultats de votre investissement :</h4>
  
                    <div className="space-y-3">
                      <div className="flex justify-between">
                        <span className="text-gray-600">Investissement initial:</span>
                        <span className="font-semibold">{formatCurrency(investmentAmount || packDetails.min_amount)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">Période:</span>
                        <span className="font-semibold text-blue-600">{packDetails.period} jours</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">Rendement quotidien:</span>
                        <span className="font-bold text-green-600">
                          {packDetails.interest_rate}% = {formatCurrency(
                            (((investmentAmount || packDetails.min_amount) * (1 + packDetails.return_percentage_40_days / 100)) / 40)
                          )}
                        </span>
                      </div>
                      <div className="flex justify-between border-t pt-3">
                        <span className="font-semibold">Total à récupérer:</span>
                        <span className="font-bold text-blue-600 text-lg">
                          {formatCurrency(
                            (investmentAmount || packDetails.min_amount) + 
                            ((investmentAmount || packDetails.min_amount) * (packDetails.interest_rate / 100) * packDetails.period)
                          )}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="font-semibold text-green-700">Bénéfice net:</span>
                  ...
                  <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-200">
                    <div className="flex items-center mb-2">
                      ...
                      <span className="font-semibold text-yellow-800">Gains Quotidiens</span>
                    </div>
                    <div className="text-center">
                  ...
                  <div className="text-center">
                    <p className="text-xs text-gray-500">
                      💡 Investissement sécurisé • Capital garanti • Régulé BCEAO
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-xl shadow-lg p-6">
                ...
                <div className="space-y-3 text-sm">
                  <div className="flex justify-between">
                    <span className="text-gray-600">Rendement quotidien:</span>
                    <span className="font-medium text-green-600">{packDetails.interest_rate}%</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Période totale:</span>
                    <span className="font-medium text-blue-600">{packDetails.period} jours</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Frais de gestion:</span>
                    <span className="font-medium text-green-600">0% (Inclus)</span>
                  </div>
                  <div className="flex justify-between">
