   ...
   const message = `🚀 Rejoins-moi sur Silvo Finance ! Rendements de 150% à 750% garantis. Utilise mon code: ${data.user.referralCode} ${referralLink}`;
    ...
    switch (method) {
      ...
      case 'sms':
        ...
        break;
      case 'email':
        window.open(`mailto:?subject=Rejoins Silvo Finance&body=${encodeURIComponent(message)}`);
        break;
    }
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-12">
          ...
          <div className="lg:col-span-2 space-y-6 md:space-y-12">
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 md:p-8">
              ...
              {/* Méthodes de partage */}
              <div>
                <h3 className="font-semibold text-gray-900 mb-3 md:mb-4 text-sm md:text-base">Partager via :</h3>
                <div className="grid grid-cols-3 md:grid-cols-5 gap-2 md:gap-4">
                  {[
                    { id: 'whatsapp', name: 'WhatsApp', icon: MessageCircle, color: 'bg-green-500' },
                    { id: 'facebook', name: 'Facebook', icon: Facebook, color: 'bg-blue-600' },
                    { id: 'twitter', name: 'Twitter', icon: Twitter, color: 'bg-blue-400' },
                    { id: 'sms', name: 'SMS', icon: Phone, color: 'bg-gray-600' },
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 md:p-8">
              ...
              <div className={`bg-gradient-to-r ${currentLevel.color} text-white p-4 md:p-6 rounded-lg mb-4 md:mb-6`}>
                <div className="flex flex-col md:flex-row md:items-center md:justify-between">
                  <div className="flex items-center space-x-3 md:space-x-4 mb-3 md:mb-0">
                    ...
                    <div>
                      ...
                      <p className="text-white/90 text-sm md:text-base">{currentLevel.commission}% de commission</p>
                    </div>
                  </div>
                ...
                <div className="bg-gray-50 p-4 md:p-6 rounded-lg">
                  ...
                  <ul className="mt-2 space-y-1">
                    {nextLevel.benefits.map((benefit, index) => (
                      <li key={index} className="text-xs md:text-sm text-gray-700 flex items-center">
              ...
              <div className="block md:hidden space-y-3">
                {data.referralHistory.map((referral) => (
                  <div key={referral.id} className="border border-gray-200 rounded-lg p-3">
                    ...
                    <div className="text-sm text-gray-600 mb-2">{referral.pack}</div>
                    <div className="flex items-center justify-between">
                      <div>
                        <div className="text-xs text-gray-500">Investissement</div>
                        <div className="text-sm font-medium">{formatCurrency(referral.investment)}</div>
                      </div>
                      <div className="text-right">
                        <div className="text-xs text-gray-500">Commission</div>
                        <div className="text-sm font-bold text-green-600">+{formatCurrency(referral.commission)}</div>
                      </div>
                    </div>
              ...
              <div className="hidden md:block overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="px-4 md:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Filleul</th>
                      <th className="px-4 md:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Pack</th>
                      <th className="px-4 md:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Investissement</th>
                      <th className="px-4 md:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Commission</th>
                      <th className="px-4 md:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Date</th>
                      <th className="px-4 md:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Statut</th>
                    </tr>
                  </thead>
          ...
          <div className="space-y-6 md:space-y-8">
            ...
            {/* Comment ça marche */}
            <div className="bg-white rounded-xl shadow-lg p-4 md:p-6">
              <h3 className="text-base md:text-lg font-bold text-gray-900 mb-3 md:mb-4">Comment ça marche ?</h3>
              ...
              <div className="space-y-3 md:space-y-4">
                <div className="flex items-start space-x-2 md:space-x-3">
                  ...
                  <div>
                    <h4 className="font-medium text-gray-900 text-sm md:text-base">Partagez votre code</h4>
                    <p className="text-xs md:text-sm text-gray-600">Invitez vos amis avec votre code unique</p>
                  </div>
                </div>
                ...
                <div className="flex items-start space-x-2 md:space-x-3">
                  ...
                  <div>
                    <h4 className="font-medium text-gray-900 text-sm md:text-base">Ils s'inscrivent</h4>
                    <p className="text-xs md:text-sm text-gray-600">Vos filleuls créent leur compte</p>
                  </div>
                </div>
                ...
                <div className="flex items-start space-x-2 md:space-x-3">
                  ...
                  <div>
                    <h4 className="font-medium text-gray-900 text-sm md:text-base">Vous gagnez</h4>
                    <p className="text-xs md:text-sm text-gray-600">Commissions versées instantanément</p>
                  </div>
                </div>
              </div>
            </div>
